...
```powershell
powershell -ExecutionPolicy Bypass -File tools/setup/setup-env.ps1 -VenvPath .3.11 -PreferredPython 3.11
```

也支持跳过其中一段：

```powershell
powershell -ExecutionPolicy Bypass -File tools/setup/setup-env.ps1 -SkipNode
powershell -ExecutionPolicy Bypass -File tools/setup/setup-env.ps1 -SkipPython
```
...
```bash
VENV_PATH=.3.11 PREFERRED_PYTHON=python3.11 bash ./tools/setup/setup-env.sh
```

也支持跳过其中一段：

```bash
SKIP_NODE=1 bash ./tools/setup/setup-env.sh
SKIP_PYTHON=1 bash ./tools/setup/setup-env.sh
```
...
- `PIPELINE_CONCURRENCY` 默认 `2`
- `SUMMARY_PIPELINE_CONCURRENCY` 默认 `2`
...
- 调度器在 `dist` 环境下会优先直接执行编译后的 `src/commands/run-video-pipeline.js`
