...
export function buildSummaryPromptInput({ pageNo, partTitle, durationSec, subtitleText, segments, promptProfile = null }) {
...
function buildBaseSystemPromptLines(pageNo) {
  return [
    ...
    "8. 每一行都要像“看点索引”，优先保留观众最可能想回看的内容：重要话题、关键知识点、核心观点、方法步骤、案例、互动、表演、突发事件、结果或结论。",
    "9. 每个时间点优先写 1 个主看点，最多补 1 个强相关次信息；不要把三四件互不相干的事硬塞进同一行。",
    "10. 默认不要把 summary 写得过短。只要字幕里有足够信息，每一行都应比极简提纲多写半步，把关键背景、动作、原因、结果或结论交代完整，但仍保持紧凑，不要拖成长句堆砌。",
    "11. 语言自然、口语化、信息密度高，直接写发生了什么、讲清了什么、得出了什么；不要写“这一段”“这里主要讲”“随后又”等空转描述。",
    "12. 不要大段复述原字幕，不要只写“闲聊”“互动”“继续聊天”“讲了个知识点”这类无法帮助定位和理解的空泛词。要尽量把“聊了什么、互动点在哪、知识点或结论是什么”写出来。",
    "13. 如果某段内容是在解释知识、观点、方法或因果关系，优先压缩成“讲了什么 + 为什么 + 结论/边界”的较完整表达；如果某段是互动、表演或事件过程，优先写清对象、动作、结果和回看价值。",
    "14. 当同一时间点下字幕信息明显较丰富时，可以把该行写得稍微展开一些，优先补足真正影响理解和回看的细节，而不是机械保持很短。",
    "15. 看到歌名、歌词、哼唱字样时，先判断是不是主播/嘉宾本人真的在唱。只有字幕能明确支持“本人在唱/接唱/清唱/唱了一段”时，才能写成“唱某歌/唱了片段”。如果更像背景音乐、伴奏、直播间外放歌曲、视频音源里飘过歌词，禁止写成主播在唱；可改写成“放了某首歌”“背景响起某首歌”，如果不构成看点也可以直接省略。",
    "16. 单独出现几句歌词、出现歌名、或者 ASR 抓到背景音乐歌词，不足以证明主播在唱；这种情况默认按背景音乐或环境音处理，除非字幕里还有明确动作或口语线索能互相印证。",
    "17. 字幕有重复、口癖、语病或 ASR 误识别时，可以整理后再表达；如果人名、歌名、梗名听不准，就改写成保守但通顺的说法。",
    `18. 交付前拿字幕和总结核对至少两遍这几个问题：单行总结必须是“<${pageNo}P> ${pageNo}#00:00 总结内容”；多行总结里每一行都必须是“${pageNo}#时间 空格 总结”，且时间要贴近对应内容开头，不能偏离太远。`,
  ];
}
