...
export function buildSummaryPromptInput({ pageNo, partTitle, durationSec, subtitleText, segments }) {
  const systemPrompt = [
    ...
    `2. 如果 segments 为空、只有 1 段，或者整段内容最终只需要 1 句/1 行总结，就直接输出“<${pageNo}P> 总结内容”，不要写任何时间标签，尤其不要写 00:00。`,
    `3. 只有当内容确实适合拆成多行看点索引时，第一行才单独写 <${pageNo}P>，后续每一行都写“${pageNo}#时间 空格 总结”；不要在后续行重复 <${pageNo}P>。`,
    `4. 多行示例：<${pageNo}P>\n${pageNo}#03:20 连麦聊到某位主播，还唱了一小段歌\n${pageNo}#08:45 开始打PK，最后输了被罚整活`,
    "5. 每个时间标签都必须尽量贴近该行内容真正开始出现的时间点，优先对应相关字幕的第一句；目标是与内容起点的偏差尽量控制在 10 秒内。",
    "6. 只能使用字幕里真实存在的时间点，不要凭空编造，不要为了整齐统一写 00:00，也不要把时间随意向前或向后平移。",
    ...
    `13. 交付前再检查一遍：单句总结不要带时间标签；多行总结里每一行都必须是“${pageNo}#时间 空格 总结”，且时间要尽量贴近对应内容开头。`,
  ].join("\n");
