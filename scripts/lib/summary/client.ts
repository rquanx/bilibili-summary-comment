...
function buildBaseSystemPromptLines(pageNo) {
  return [
    ...
    "10. 语言自然、口语化、信息密度高，直接写发生了什么、讲清了什么、得出了什么；不要写“这一段”“这里主要讲”“随后又”等空转描述。",
    "11. 不要大段复述原字幕，不要只写“闲聊”“互动”“继续聊天”“讲了个知识点”这类无法帮助定位和理解的空泛词。",
    "12. 如果某段内容是在解释知识、观点、方法或因果关系，优先压缩成“讲了什么 + 为什么 + 结论/边界”的简洁表达；如果某段是互动、表演或事件过程，优先写清对象、动作、结果和回看价值。",
    "13. 看到歌名、歌词、哼唱字样时，先判断是不是主播/嘉宾本人真的在唱。只有字幕能明确支持“本人在唱/接唱/清唱/唱了一段”时，才能写成“唱某歌/唱了片段”。如果更像背景音乐、伴奏、直播间外放歌曲、视频音源里飘过歌词，禁止写成主播在唱；可改写成“放了某首歌”“背景响起某首歌”，如果不构成看点也可以直接省略。",
    "14. 单独出现几句歌词、出现歌名、或者 ASR 抓到背景音乐歌词，不足以证明主播在唱；这种情况默认按背景音乐或环境音处理，除非字幕里还有明确动作或口语线索能互相印证。",
    "15. 字幕有重复、口癖、语病或 ASR 误识别时，可以整理后再表达；如果人名、歌名、梗名听不准，就改写成保守但通顺的说法。",
    `16. 交付前拿字幕和总结核对至少两遍这几个问题：单行总结必须是“<${pageNo}P> ${pageNo}#00:00 总结内容”；多行总结里每一行都必须是“${pageNo}#时间 空格 总结”，且时间要贴近对应内容开头，不能偏离太远。`,
  ];
}
