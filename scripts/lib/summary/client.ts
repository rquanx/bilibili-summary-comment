...
export function buildSummaryPromptInput({ pageNo, partTitle, durationSec, subtitleText, segments }) {
  const systemPrompt = [
    ...
    "8. 每一行都要像“看点索引”，优先保留粉丝最可能想回看的内容：连麦对象、明确发生的唱歌/表演、PK/惩罚/整活、游戏或现场动作、重要观点、突发事件、结果。",
    ...
    "12. 看到歌名、歌词、哼唱字样时，先判断是不是主播/嘉宾本人真的在唱。只有字幕能明确支持“本人在唱/接唱/清唱/唱了一段”时，才能写成“唱某歌/唱了片段”。如果更像背景音乐、伴奏、直播间外放歌曲、视频音源里飘过歌词，禁止写成主播在唱；可改写成“放了某首歌”“背景响起某首歌”，如果不构成看点也可以直接省略。",
    "13. 单独出现几句歌词、出现歌名、或者 ASR 抓到背景音乐歌词，不足以证明主播在唱；这种情况默认按背景音乐或环境音处理，除非字幕里还有明确动作或口语线索能互相印证。",
    "14. 字幕有重复、口癖、语病或 ASR 误识别时，可以整理后再表达；如果人名、歌名、梗名听不准，就改写成保守但通顺的说法。",
    `15. 交付前拿字幕和总结核对至少两遍这几个问题：单行总结必须是“<${pageNo}P> ${pageNo}#00:00 总结内容”；多行总结里每一行都必须是“${pageNo}#时间 空格 总结”，且时间要贴近对应内容开头，不能偏离太远。`,
  ].join("\n");
