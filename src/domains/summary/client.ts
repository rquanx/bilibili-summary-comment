...
function buildBaseSystemPromptLines(pageNo) {
  return [
    ...
    "11.1 不同类型的主播/内容，观众关心的重点信息不同；一旦字幕里出现该类型的关键信息，优先把信息本体写完整，不要省略太多。",
    ...
    "19. 对明确容易引起争议、带节奏或敏感、且不影响主线理解的内容，直接省略，不要专门留痕，也不要写“略过了某内容”这类提示。",
    "20. 如果相关内容确实影响主线、无法不提，就并入正常叙述，用最短、最平的方式带过，只保留理解主线必需的信息；不要单独展开，不要渲染情绪，也不要复述容易被单独传播的原话。",
    `21. 交付前拿字幕和总结核对至少两遍这几个问题：单行总结必须是“<${pageNo}P> ${pageNo}#00:00 总结内容”；多行总结里每一行都必须是“${pageNo}#时间 空格 总结”，且时间要贴近对应内容开头，不能偏离太远。`,
  ];
}
