...
完整流水线由 [`scripts/commands/run-video-pipeline.mjs`](./scripts/commands/run-video-pipeline.mjs) 驱动：
...
```bash
node scripts/commands/get-bili-top-comment.mjs --cookie-file ./cookie.txt --bvid BVxxxxxxxxxx
```

### 6. 手工发送一份总结

```bash
node scripts/commands/post-bili-summary.mjs --cookie-file ./cookie.txt --bvid BVxxxxxxxxxx --message-file work/BVxxxxxxxxxx/pending-summary.md
```
...
```bash
node scripts/commands/refresh-bili-cookie.mjs --access-token <token> --refresh-token <token>
```
...
- `scripts/commands/run-video-pipeline.mjs`：总入口，负责同步、字幕、总结、发布
- `scripts/commands/sync-bili-video-state.mjs`：只做视频与分 P 状态同步
- `scripts/commands/import-summary-file.mjs`：将已有总结文件导入 SQLite
- `scripts/commands/publish-pending-summaries.mjs`：发布未发布总结，必要时重建线程
- `scripts/commands/post-bili-summary.mjs`：底层评论发布脚本
- `scripts/commands/login-bili-tv.mjs`：通过 TV 二维码登录获取可刷新的授权
- `scripts/commands/refresh-bili-cookie.mjs`：用 refresh token 刷新 cookie
- `scripts/commands/sync-summary-users.mjs`：扫描指定用户最近投稿并逐个执行流水线
- `scripts/commands/run-scheduler.mjs`：常驻调度入口
- `scripts/maintenance/cleanup-work-files.mjs`：清理过期工作目录
