...
- `scripts/commands/run-video-pipeline.ts`：总入口，负责同步、字幕、总结、发布
- `scripts/commands/sync-bili-video-state.ts`：只做视频与分 P 状态同步
- `scripts/commands/import-summary-file.ts`：将已有总结文件导入 SQLite
- `scripts/commands/publish-pending-summaries.ts`：发布未发布总结，必要时重建线程
- `scripts/commands/post-bili-summary.ts`：底层评论发布脚本
- `scripts/commands/login-bili-tv.ts`：通过 TV 二维码登录获取可刷新的授权
- `scripts/commands/refresh-bili-cookie.ts`：用 refresh token 刷新 cookie
- `scripts/commands/sync-summary-users.ts`：扫描指定用户最近投稿并逐个执行流水线
- `scripts/commands/run-scheduler.ts`：常驻调度入口
- `scripts/maintenance/cleanup-work-files.ts`：清理过期工作目录
